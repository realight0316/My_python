...
def solution(lines):
    ...
    for log in lines:
        ss = in_sec(log[11:])               # 초(세컨드) 추출
        ms = int(float(log[24:-1]) * 1000)  # 밀리초(밀리세컨드) 추출
        start = ss - ms + 1                 # 시작시간과 끝나는 시간을 포함하기 때문에 +1 [예: 23~25는 2가 아니라 3]
        end = ss
        secline.append([start, end])        # 시작시간과 끝나는 시간을 알고있는 리스트


    for idx, mslog in enumerate(secline):
        temp = 0

        print(f"{mslog[0]}, {mslog[1]} // {mslog[1]+999}")  # 진행 확인용, 없어도 무방

        for i in range(idx, len(secline)):      # 현재 요청 이후의 모든 요청들과 대조 (현 로그 이후 모든 로그는 자신보다 늦게 끝남)
            if mslog[1]+1000 > secline[i][0]:   # 초당처리량이므로 현재 요청 종료 시점 + 1000 보다 먼저 시작한 작업은 동시작업
                temp += 1
        answer = max(answer, temp)              # 최대갯수 정답저장
