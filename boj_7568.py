...
for _ in range(case_num):
    x,y = map(int, input().split()) # 키/몸무게 케이스 수집
    temp = [x, y]
    case.append(temp)

for i in range(case_num):           # 기준
    temp = 0
    for j in range(case_num):       # 비교대상
        x, y = map(int, case[i])
        p, q = map(int, case[j])
        if x < p and y < q:         # 기준이 된 케이스와 비교대상의 키/몸무게 비교
            temp += 1               # 나보다 큰 대상의 수
    ans.append(temp+1)              # 큰 대상의 수 + 1 = 순위

for a in ans:                       # 출력양식 맞추기
    print(a, end=' ')